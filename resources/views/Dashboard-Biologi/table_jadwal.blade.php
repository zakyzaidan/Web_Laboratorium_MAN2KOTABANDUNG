...
$(document).ready(function() {
    ...
    $(document).on('click', '.edit-button', async function() {
        var id = $(this).data('id');
        try {
            const response = await fetch('/jadwal-praktikum-biologi/' + id + '/edit');
            if (!response.ok) {
            ...
            $('#form').attr('action', '/jadwal-praktikum-biologi/' + id);
    ...
    $(document).on('click', '.delete-button', function() {
        if (confirm('Are you sure you want to delete this item?')) {
            ...
            $.ajax({
                type: 'POST',
                url: '/jadwal-praktikum-biologi/' + id,
                data: {
                    _token: '{{ csrf_token() }}',
                    _method: 'DELETE'
                },
    ...
    try {
        const response = await fetch('/jadwal-praktikum-biologi/check-date/' + jadwal_praktikum);
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
        const data = await response.json();
